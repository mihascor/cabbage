...
PAGE_TIMEOUT_MS = 30000  # максимальное время ожидания загрузки страницы
RETRY_COUNT = 3  # количество попыток открыть страницу
WAIT_AFTER_LOAD_MS = 3000 # время ожидания после загрузки страницы, чтобы данные точно отобразились
...
async def extract_data(page, url: str) -> Dict[str, int]:
    """
    Пытается открыть страницу несколько раз при таймаутах.
    Если после RETRY_COUNT попыток не удалось — кидает исключение выше.
    """
    ...
    for attempt in range(1, RETRY_COUNT + 1):
        try:
            ...
            # ВАЖНО: полностью перезагружаем страницу перед новой попыткой
            try:
                await page.close()
    ...
    raise RuntimeError(f"Не удалось загрузить страницу после {RETRY_COUNT} попыток: {last_error}")
